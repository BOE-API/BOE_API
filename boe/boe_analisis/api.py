...
class EstimatedCountPaginator(Paginator):
    ...
    def get_next(self, limit, offset, count):
        print "COUNT"
        print self.count
        if limit + offset > self.count and limit + offset >= self.get_max_id():
...
class BOEResource(DocumentoResource):
    ...
    class Meta:
        queryset = Documento.objects.exclude(url_xml=None).filter(diario = 'BOE').order_by('fecha_publicacion')
...
class BORMEResource(DocumentoResource):
    ...
    class Meta:
        queryset = Documento.objects.exclude(url_xml=None).filter(diario = 'BORME').order_by('fecha_publicacion')
